...
### 읽기 기록 — 가벼운 게이미피케이션 (2026-09-11)

- 재료는 기존 읽음 표시(`pw-read`, `pw-sprint-read`)뿐이다. 새 저장소 키도,
  타임스탬프도, 백엔드도 없다. 기록은 이 브라우저 안에만 남는다.
- 지표는 실제 값만 쓴다: 누적 읽은 분(레벨), 최신 발행일부터 하루라도 하나 읽은
  연속 발행일(연속 읽기), 읽은 편수(배지·시리즈 진행). 꾸며 낸 점수는 없다.
- 레벨은 누적 읽은 분으로 계산한다. L레벨 역치는 15·(L−1)·(L+2)분 — 60, 150,
  270, 420분 순으로 벌어지고 상한은 없다.
- 표면은 세 곳: `/record/` 기록 페이지(헤더 내비 "기록"), 홈 머리의 기록 한 줄,
  읽기 화면 이전·다음 아래의 기록 줄. 읽음 마킹 순간 `pw:marked` 문서 이벤트로
  바로 갱신되고, 다른 탭은 storage 이벤트를 따른다.
- 읽기 화면의 기록 줄은 보상을 미리 보여 준다 — "끝까지 읽으면 +N분". 읽음
  상태는 초록 체크 아이콘과 함께 나타낸다(색만으로 상태를 말하지 않는다).
- 축하 장치는 없다. 토스트·폭죽·진입 애니메이션·그림자 없이, 경계와 숫자와
  아이콘만으로 조용히 누적을 보여 준다. 해금된 배지 아이콘은 `--color-good`.
- 배지 19개는 고정 정의(`src/lib/record.ts`)이고 해금 여부만 지표에서 유도한다.
  조건이 바뀌면 정의 배열 한 곳만 고친다.

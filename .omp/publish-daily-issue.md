...
독자가 GitHub 이슈 템플릿으로 칼럼을 요청하고, 소유자가 검토해 `publish` 라벨을
붙인다 — 라벨이 곧 발행 승인이다. 템플릿은 라벨을 자동으로 붙이지 않으므로,
라벨이 붙은 열려 있는 이슈는 이미 승인된 요청뿐이다. 이 태스크는 승인된 이슈를
전부 읽어 각각을 저작 규칙(CONTENT.md)에 맞는 완성된 칼럼으로 발행하고,
...
- 1건 이상 → 오래된 것부터 처리. 기본은 **1 이슈 = 1 호**.
  ...
  - 라벨이 없는 요청 이슈는 발행 대상에서 제외된다 — 승인되지 않은 요청이다.
